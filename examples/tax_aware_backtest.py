...
# -------------------------------------------------------------
# Short-Term Capital Gains Tax Rate (held <= 365 days):
# Federal: 37.0% + NIIT (Obamacare): 3.8% + CA State Income Tax: 14.4% = 55.2%
STCG_TAX_RATE = 0.552

# Long-Term Capital Gains Tax Rate (held > 365 days):
# Federal: 20.0% + NIIT (Obamacare): 3.8% + CA State Income Tax: 14.4% = 38.2%
LTCG_TAX_RATE = 0.382

# Transaction fee: 0.1% execution fee
FEE_RATE = 0.001

def run_tax_aware_backtest():
    print("--- Starting California Tax-Aware DCA Backtest Simulation ---")
    ...
    # DCA setup
    start_idx = 504  # Start after 2 years of lookback
    rebalance_freq = 21
    dca_amount = 2000.0
    
    # -------------------------------------------------------------
    # Simulation Data Structures for 3 Strategies:
    # 1. Standard Rebalance (Taxable)
    # 2. Buy-Only Rebalance (Taxable)
    # 3. Standard Rebalance (Tax-Free / e.g. IRA)
    # -------------------------------------------------------------
    
    # Track daily total values
    taxable_std_value = []
    taxable_buy_only_value = []
    tax_free_value = []
    cash_savings_value = []
    
    # Track actual tax lots for taxable portfolios
    # Lot: {'date': datetime, 'shares': float, 'buy_price': float}
    taxable_std_lots = {t: [] for t in stock_tickers}
    taxable_buy_only_lots = {t: [] for t in stock_tickers}
    
    # Track raw dollar balances
    ...
    print(f"\nSimulating DCA of ${dca_amount:,.2f} monthly from {returns_df.index[start_idx].date()} to {returns_df.index[-1].date()}...")
    ...
    for i in range(start_idx, n_days):
        ...
        # 1. Tax-Free compounds naturally
        tax_free_holdings = tax_free_holdings * (1.0 + daily_ret)
        
        # 2. Cash compounds at 4%
        ...
        if (i - start_idx) % rebalance_freq == 0:
            ...
            # -----------------------------------------------------------------
            # Calculate current total holdings value before deposit
            ...
            # Calculate current values per stock
            ...
            # We must execute Sells first (calculating taxes)
            sells_cash = 0.0
            sells_fee = 0.0
            sells_tax = 0.0
            ...
            for t in stock_tickers:
                ...
                if diff < 0:  # SELL needed
                    ...
                    # Identify specific lots to sell using HIFO (Highest-In, First-Out)
                    lots = taxable_std_lots[t]
                    # Sort lots by buy price descending
            ...
            # -----------------------------------------------------------------
            # STRATEGY 2: Buy-Only Rebalance (Taxable) - NO SELLS
            # -----------------------------------------------------------------
            # Calculate current total holdings value before deposit
            ...
            # -----------------------------------------------------------------
            # STRATEGY 3: Standard Rebalance (Tax-Free - e.g. IRA)
            # -----------------------------------------------------------------
    ...
    print("\n--- California Tax-Aware DCA Simulation Summary ---")
    print("-----------------------------------------------------")
    pd.set_option('display.max_columns', None)
    pd.set_option('display.width', 1000)
    print(metrics_df)
    print(f"\nTotal California/Federal Capital Gains Tax Paid (Standard Rebalance): ${total_tax_paid_std:,.2f}")
    ...
    plt.title('Monthly DCA Simulation ($2,000/Month): Strategy % ROI Comparison (California Tax)', fontsize=14, fontweight='bold')
    ...
    output_path = os.path.join(script_dir, "tax_aware_performance.png")
    plt.savefig(output_path, bbox_inches='tight')
    plt.close()
    
    print(f"\nTax-aware performance chart saved to: {output_path}")

if __name__ == "__main__":
    run_tax_aware_backtest()
